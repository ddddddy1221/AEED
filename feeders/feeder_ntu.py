    ...
    def load_data(self):
        ...
        if self.split == 'train':
            ...
            with open(self.train_id_path, 'r') as f:
                train_name = [line.strip() for line in f if line.strip()]
            with open(self.sample_name_path, 'r') as f:
                sample_name_list = [
                    line.strip() for line in f.readlines()
                ]
            name2idx = {name: i for i, name in enumerate(sample_name_list)}

            self.sample_name = train_name  # 保存字符串名字
            self.idx = [i for i, n in enumerate(train_name) if n in ske_list]
