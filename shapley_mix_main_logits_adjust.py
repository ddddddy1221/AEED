...
class LogitAdjust(nn.Module):
    def __init__(self, cls_num_list, tau=1, weight=None, beta=0.1, entropy_threshold=0.5):
        super(LogitAdjust, self).__init__()
        # ... [原有代码不变] ...

        # 熵驱动参数
        self.beta = beta  # 熵补偿强度系数
        self.entropy_threshold = entropy_threshold  # 熵激活阈值
    ...
    def forward(self, x, target):
        ...
        max_entropy = torch.log(torch.tensor(self.num_classes, device=x.device))
        normalized_entropy = entropy / max_entropy  # 归一化到0-1

        # 2. 熵驱动的动态补偿
        entropy_factor = torch.where(
            normalized_entropy > self.entropy_threshold,
            self.beta * (normalized_entropy - self.entropy_threshold),
            torch.zeros_like(normalized_entropy)
        ).unsqueeze(1)  # [batch_size, 1]

        # 3. 应用动态补偿
        dynamic_m_list = self.m_list * (1 + entropy_factor)
        x_m = x + dynamic_m_list
...
def get_parser():
    ...
    parser.add_argument(
        '--config',
        default='./config/nturgbd-cross-subject/long_tailed_logit_adjust.yaml',
...
class DyCOLTScheduler:
    def __init__(self, max_epoch):
        self.max_epoch = max_epoch

    def get_lambda(self, epoch):
        base = 0.2 + 0.8 * (5 * epoch / self.max_epoch) / (1 + 5 * epoch / self.max_epoch)
        noise = np.random.normal(0, 0.1)
        return np.clip(base + noise, 0.2, 1.0)

    def get_p_mix(self, epoch):
        return 0.5 * (1 - math.cos(math.pi * epoch / self.max_epoch))
...
class Processor():
    ...
    def __init__(self, arg):
        self.arg = arg
        self.save_arg()
        self.part_distri = torch.zeros(self.arg.model_args['num_class'], 10+10).cuda() + 1e-6#label_num, part_combination_num C(5,2) + C(5,3)  # 60 20
        if arg.phase == 'train':
    ...
    def load_data(self):
        ...
        if self.arg.phase == 'train':
            ...
            self.data_loader['train'] = torch.utils.data.DataLoader(
                dataset=train_d,
                batch_size=self.arg.batch_size,  # 直接使用完整 batch_size
                shuffle=True,  # 启用随机打乱
    ...
    def train_yuan(self, epoch, save_model=False):
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            self.global_step += 1
            with torch.no_grad():
                data = data.float().cuda(self.output_device)# 4 3 64 25 2   N C T V M
                label = label.long().cuda(self.output_device) # 4 ,, 40 8 44 41
            timer['dataloader'] += self.split_time()
            ################ part dist estimation ####################
            #tag: part dist estimation
            ...
            if epoch < self.arg.naive_mix_epoch or batch_idx % estimation_interval == 0:
                ...
                spa_list = random.sample(list(range(5)), Cs)#从0到4中随机选择Cs个索引
                spa_label = self.map_list2label(spa_list)# 选择的部件列表映射为一个标签，标识选择了哪些部件的编码
                # 前warm_epoch个epoch使用naive方法，之后使用shapley
                ...
                part_each_num = np.array([5,6,6,4,4])#每个部分的关节数量
                weight = weight/part_each_num[spa_list].sum()*10 # node normlization (*10 just for print debug)
                self.update_part_dist(epoch, self.global_step, spa_label, weight, label)#  更新部件分布
            ##########################################################
            # self.part_distri.shape = (60, 20)
            # mix generation  混合数据生成
            ...
            output = self.model(torch.cat([data,mix_data],dim=0))# 8 60
    ...
    def train(self, epoch, save_model=False):
        ...
        train_labels = self.data_loader['train'].dataset.label
        class_counts = np.bincount(train_labels)

        # 识别尾部类（样本数最少的20%类别）
        sorted_classes = np.argsort(class_counts)
        tail_classes = sorted_classes[:int(0.2 * len(class_counts))].tolist()

        # 设置当前epoch的观测比例（从0.2线性增加到1.0）
        lambda_current = 0.2 + 0.8 * (epoch / self.arg.num_epoch)
        self.data_loader['train'].dataset.set_lambda(lambda_current)
        self.model.train()
        self.print_log('Training epoch: {}'.format(epoch + 1))
        loader = self.data_loader['train']
        self.adjust_learning_rate(epoch)
        ...
        if self.arg.local_rank<=0:
            self.train_writer.add_scalar('epoch', epoch, self.global_step)
            self.train_writer.add_scalar('lambda', lambda_current, self.global_step)
        self.record_time()
        timer = dict(dataloader=0.001, model=0.001, statistics=0.001)
        # loader.sampler.set_epoch(epoch)
        process = tqdm(loader, ncols=40)
        ...
        for batch_idx, (data, label, index) in enumerate(process):
            ...
            with torch.set_grad_enabled(True):
                # 计算Shapley值（使用高效近似算法）
                sv_grad = weight_calcu_shapley(
                    data, self.model, self.loss, label,
                    num_samples=8,  # 减少采样次数加速计算
                    abs_val=True  # 使用绝对值增强稳定性
                )
            # ======== 2. 设置给Feeder（用于下一个batch的显著性裁剪） ========

            self.data_loader['train'].dataset.set_sv_grad(sv_grad.cpu().numpy())
            # ======== 3. 梯度感知混合增强（GradShapMix） ========
            if epoch >= 10:  # 前10个epoch不使用混合增强（模型预热）
                data = ske_swap_gradshapmix(
                    data, sv_grad, label, tail_classes,
                    epsilon=0.1, model=self.model, criterion=self.loss
                )
            ...
            if self.arg.local_rank <= 0:
                self.train_writer.add_scalar('acc', acc, self.global_step)
                self.train_writer.add_scalar('loss', loss1.data.item(), self.global_step)
        ...
        self.print_log(
            '\tMean training loss: {:.4f}. Mean training acc: {:.2f}%. Lambda: {:.2f}'.format(
                np.mean(loss_value), np.mean(acc_value) * 100, lambda_current
            )
        )
        self.print_log('\tTime consumption: [Data]{dataloader}, [Network]{model}'.format(**proportion))
        ...
        if save_model and self.arg.local_rank <= 0:
            ...
            torch.save(weights,
                       self.arg.model_saved_name + '-' + str(epoch + 1) + '-' + str(self.current_acc) + '.pt')
...
if __name__ == '__main__':
    ...
    arg.local_rank = 0
    print('local_rank: ', arg.local_rank)
